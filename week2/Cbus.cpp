...
int best_sol=10000; // lưu độ dài cho tuyến đường ngắn nhất
int C[1000][1000]; // ma trận biểu diễn khoảng cách giữa 2 điểm
int appear[1000]; // đã đi qua điểm i hay chưa
int cmin=100000; // khoảng cách nhỏ nhất giữa 2 điểm
int load=0; // số khách hiện tại của xe
int sol; // Đồ dài quảng đường đã đi
int X[1000]; // Lưu các điểm đã đi qua 

// kiểm tra xem khách v đã xuất hiện hay chưa hoặc lên xe có vượt trọng tải của xe hay k
int check(int v){
	if(load==k&&v<=n ) return 0; // can cho len nhung day
	if(appear[v]==1) return 0; // da di qua
	if(v>n&&appear[v-n]==0) return 0; //  la diem xuong nhung khach chua len
	return 1;
}
...
void Try(int t){
	if(sol+cmin*(2*n-t+2)>best_sol) return;
	for(int v=1;v<=2*n;v++){
        cout << check(v);
		if(check(v)){
			X[t]=v;
