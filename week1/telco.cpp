...
clock_t start, end;
    ...
    while (getline(ifs, line) && line != "#") {
        ...
        if (is_valid_number(from_number) && is_valid_number(to_number)) {          
            total_duration[from_number]+=toSecond(from_time,to_time); // cộng thêm thời gian gọi
            phoneCount[from_number]++; // tăng số lần gọi lên 1
            CountCall++;
...
// độc second block
    while (getline(ifs, line) && line != "#") {
        ...
        if (cmd == "?check_phone_number") {
            if(CountLine == CountCall) // nếu số dòng == số cuộc gọc hợp lệ thì tất cả số điện thoại đều chính xác
            cout << 1 << endl;
